...
from model import IncrementalClassifier, RegressionHead
...
def experiment(config: DictConfig):
    ...
    is_regression = hasattr(method.module, 'head') and isinstance(method.module.head, RegressionHead)
    ...
    for task_id, (train_task, test_task) in enumerate(zip(train_tasks, test_tasks)):
        ...
        with fabric.init_tensor():
            for epoch in range(config.exp.epochs):
                ...
                train(method, train_task, task_id, log_per_batch, is_regression)
                metric = test(method, test_task, task_id, gen_cm, log_per_batch, is_regression)
                if calc_fwt:
                    method_tmp = Composer(
                    ...
                    train(method_tmp, train_task, task_id, log_per_batch, is_regression, quiet=True)
                    b[task_id] = test(method_tmp, test_task, task_id, gen_cm, log_per_batch, is_regression, quiet=True)
                if lastepoch:
                    R[task_id, task_id] = metric
                if task_id > 0:
                    for j in range(task_id-1, -1, -1):
                        metric = test(method, test_tasks[j], j, gen_cm, log_per_batch, is_regression, cm_suffix=f' after {task_id}')
                        if lastepoch:
                            R[task_id, j] = metric
        metric_name = 'avg_mse' if is_regression else 'avg_acc'
        wandb.log({metric_name: R[task_id, :task_id+1].mean()})
...
def train(method: MethodPluginABC, dataloader: DataLoader, task_id: int, log_per_batch: bool, is_regression: bool = False, quiet: bool = False):
    """
    Train one epoch.
    """
...
def test(method: MethodPluginABC, dataloader: DataLoader, task_id: int, gen_cm: bool, log_per_batch: bool, is_regression: bool = False, quiet: bool = False, cm_suffix: str = '') -> float:
    """
    Test one epoch.
    """
    ...
    with torch.no_grad():
        if is_regression:
            ...
            if not quiet:
                log.info(f'MSE of the model on the test data (task {task_id}): {avg_mse:.6f}')
                wandb.log({f'Loss/test/{task_id}': avg_loss})
                wandb.log({f'MSE/test/{task_id}': avg_mse})
